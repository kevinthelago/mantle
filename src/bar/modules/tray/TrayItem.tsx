...
function ItemIcon({ item }: { item: TrayItem }): React.ReactElement {
  if (item.icon?.startsWith('data:') || item.icon?.startsWith('file://')) {
    return <img className={styles.icon} src={item.icon} alt={item.title} draggable={false} />
  }
...
function MenuPopover({
  ...
  return (
    <div className={styles.menu} style={style} role="menu">
      <MenuItemList items={menu.items} itemKey={itemKey} onEvent={onEvent} onClose={onClose} />
    </div>
  )
}
  ...
  return (
    <>
      {items
        .filter((i) => i.visible)
        .map((i) => (
          <MenuItemView key={i.id} item={i} itemKey={itemKey} onEvent={onEvent} onClose={onClose} />
        ))}
    </>
  )
}
...
function MenuItemView({
  ...
  return (
    ...
    >
      {checkMark !== null && <span className={styles.menuCheck}>{checkMark}</span>}
