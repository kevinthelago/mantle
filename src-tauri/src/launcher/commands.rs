...
#[tauri::command]
pub async fn launch_app(id: String, state: State<'_, LauncherState>) -> CmdResult<()> {
    let svc = state.0.read().await;
    svc.launch(&id).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn run_command(cmd: String, state: State<'_, LauncherState>) -> CmdResult<()> {
    let svc = state.0.read().await;
...
#[tauri::command]
pub async fn get_history(state: State<'_, LauncherState>) -> CmdResult<Vec<(String, u32)>> {
    let svc = state.0.read().await;
    Ok(svc.usage_counts())
}
